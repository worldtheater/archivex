...
fun Modifier.scrollbar(
    ...
    return drawWithContent {
        ...
        if (state.isScrollInProgress || alpha > 0f) {
            ...
            val contentSize = viewportSize + state.maxValue

            if (contentSize <= viewportSize) return@drawWithContent
            ...
            val knobPosition =
                if (state.maxValue > 0)
                    (state.value / state.maxValue.toFloat()) * (viewportSize - knobSize) + padding.toPx()
                else
                    padding.toPx()
